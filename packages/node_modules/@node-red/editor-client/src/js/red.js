...
var RED = (function() {
    ...
    async function loadFlows(done) {
        try {
            ...
            const deployedNodesPromise = $.ajax({
                headers: {
                    "Accept": "application/json"
                },
                cache: false,
            ...
            const [draftedNodes, deployedNodes] = await Promise.all([draftedNodesPromise, deployedNodesPromise]);

            const diffNodeIds = RED.utils.diffNodes(draftedNodes.flows,deployedNodes.flows);
            let isDeployed = true;
            // if (diffNodeIds.size != 0) {
            //   isDeployed = false;
            //   RED.notify(
            //     '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="10" height="10"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512z" fill="rgb(0, 188, 255)"/></svg> &nbsp; represendts drafted nodes',
            //     'html'
            //   );
            // }

            let nodes = deployedNodes;
            RED.nodes.setDraftDirty(false);
            RED.nodes.setDeleteDraftDirty(false);
            RED.nodes.dirty(false);

            if (draftedNodes.success && draftedNodes.flows.length > 0) {
                nodes = draftedNodes;
            ...
            if (nodes) {
                ...
                try {
                    nodes.flows.map(flow => flow.isDraft = !!diffNodeIds.has(flow.id))
    ...
    function buildMainMenu() {
        ...
        $('<li><a id="red-ui-header-button-sidemenu" class="button" href="#"><i class="fa fa-bars" style="color: black;"></i></a></li>').appendTo(".red-ui-header-toolbar")
    ...
    function removeQueryParam(param) {
        ...
        window.history.replaceState({}, document.title, url); // Update the URL without redirecting
    }
    ...
    async function loadEditor() {
        ...
        const showDiff = params.get('showDiff');
        let loadDiffView = false;
        if (showDiff === "true") {
            removeQueryParam("showDiff");
            loadDiffView = true;
        }
            ...
            if (theme.header) {
                ...
                if (theme.header.title) {
                    const header = $('<h1>')
                        ...
                        .css({
                            "color": "black",
                            "font-family": "'Inter', sans-serif", // Directly using the Inter font
                            "font-size": "1.5rem",
                            "font-weight": "800",
                            "text-align": "left",
