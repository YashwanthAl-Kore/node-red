...
RED.nodes = (function() {
    ...
    var deleteDraftDirty = false;

    function setDirty(d) {
        // sets both deploy and draft to dirty
        var activeWorkspace = RED.workspaces.active();

        let currentTab = document.getElementById('red-ui-tab-' + activeWorkspace);

        if (currentTab) {
            // Create SVG element
            let svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
            svg.setAttribute("class", "red-ui-flow-tab-changed red-ui-flow-node-changed");
            ...
            // Append SVG to the current tab
            currentTab.appendChild(svg);
        } else {
            console.error("Element not found: red-ui-tab-" + activeWorkspace);
        }
    ...
    function setDraftDirty(state){
        RED.events.emit("workspace:draftDirty",{ draftDirty: state });
        draftDirty = state;
        return draftDirty;
    }

    function setDeleteDraftDirty(state){
        RED.events.emit("workspace:deleteDraftDirty", {deleteDraftDirty: state});
        deleteDraftDirty = state;
        return deleteDraftDirty;
    }
    ...
    return {
        ...
        setDeleteDraftDirty: function(state) {
            if (state == null) {
                return deleteDraftDirty;
            } else {
                setDeleteDraftDirty(state);
            }
        },
    };
})();
