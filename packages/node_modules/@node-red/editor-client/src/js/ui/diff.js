RED.diff = (function() {
    ...
    var diffList;
    let showRemoteDiffRecursiveCall = 0;
    
    function init(loadDiffView) {
     ...
     * 1. Local flows vs. deployed flows (preferred if local flows are available).
     * 2. Drafted flows vs. deployed flows (fallback if local flows are not available).
     * 3. If neither local flows nor drafted flows are available, no diff is shown, and the function returns.
     */
    async function getRemoteDiff(callback) {
        const appId = RED.utils.getAppId();
        const appType = RED.utils.getAppType();
        let deployedFlows;
        let draftedFlows;
        let localFlows = RED.nodes.createCompleteNodeSet();
        if(!Array.isArray(localFlows) || !localFlows.length){
            RED.events.on('flows:loaded', () => {
                if(showRemoteDiffRecursiveCall > 4){
                    throw Error("Error while showing remote diff unable to get localFlows stage")
                }
                showRemoteDiffRecursiveCall += 1;
                showRemoteDiff();
            })
            console.log("RemoteDiff cancelled: Flows are not yet loaded on editor")
            return;
        }

        try {
            const [deployedResponse, draftedResponse] = await Promise.all([
                $.ajax({
                  headers: {
                    Accept: 'application/json'
                  },
                  cache: false,
                  url: `flows?appId=${appId}&appType=${appType}`
                }),
                $.ajax({
                  headers: {
                    Accept: 'application/json'
                  ...
                  cache: false,
                  url: `drafts?appId=${appId}&appType=${appType}`
                })
              ]);
            
              // Assign results to variables
              deployedFlows = deployedResponse;
              draftedFlows = draftedResponse;
        } catch (error) {
          console.error(
            'Error while making api call to get remote diff',
            error
          );
          return;
        }
        let activeFlows = localFlows;
        let remoteFlows = deployedFlows.flows;
        if(activeFlows.length == 1 && activeFlows[0].type == 'tab' && activeFlows[0]?.label.split(" ")[0] === "Flow"){
            if(Array.isArray(draftedFlows.flows) && draftedFlows.flows.length === 0){
                return;
            }
            activeFlows = draftedFlows;
        }
        // NOTE: to sortObjByKeys and add ignore list for showDiff
        activeFlows = activeFlows.map(flow => {
          const { isPopulated, options, props, isDraft, appType, ...rest } = flow;
          return sortObjByKeys(rest);
        });
        remoteFlows = remoteFlows.map(flow => {
          const { isPopulated, options, props, isDraft, appType, ...rest } = flow;
          return sortObjByKeys(rest);
        });
        var localDiff = generateDiff(remoteFlows, activeFlows);
        var remoteDiff = generateDiff(remoteFlows, remoteFlows);
        if(Object.keys(localDiff.added) == 0 &&
            Object.keys(localDiff.deleted) == 0 &&
            Object.keys(localDiff.changed) == 0 &&
            Object.keys(localDiff.positionChanged) == 0 &&
            Object.keys(localDiff.moved) == 0)
        {
            console.log("Their are no changes to show")
            // RED.notify('<p>' + RED._("deploy.successfulDeploy") + '</p>', "success");
        }
        remoteDiff.rev = deployedFlows.rev;
        localDiff.rev = deployedFlows.rev;
    ...
    function showRemoteDiff(diff, options = {}) {
        if (!diff) {
            getRemoteDiff((remoteDiff) => {
                return showRemoteDiff(remoteDiff, options)
            });
        } else {
    ...
    function showDiff(diff, options) {
        ...
        const appId =  RED.utils.getAppId();
        const appType = RED.utils.getAppType();
        const draftedNodesPromise = $.ajax({
            headers: {
                "Accept":"application/json"
            },
            cache: false,
        ...
        //             click: function() {
        //                 // if active changes are their in tab deploy them 
        //                 // else if drafted changes are their deploy them
        ...
        //                     }
        //                     (async()=>{
        //                         let activeNodes = RED.nodes.createCompleteNodeSet();
        //                         const draftedData = await draftedNodesPromise;
        //                         let reload = false;
        //                         if(activeNodes.length == 1 && activeNodes[0].type == 'tab' && draftedData  && draftedData.flows){
        //                             reload = true;
        //                             activeNodes = draftedData.flows
        //                         }
        //                         const data = { flows: activeNodes, rev : RED.nodes.version() };
        //                         const saveNodesPromise =  $.ajax({
        //                             url: `flows?appId=${appId}`,
        //                             type: "POST",
        //                             data: JSON.stringify(data),
        //                             contentType: "application/json; charset=utf-8"
        //                         })
        //                         await saveNodesPromise;
        //                         // close the iframe window after merge and deploy
        //                         console.log("SENT ",{action:"INTEGRATION:UI:POSTED_FLOWS"});
        //                         window.parent.postMessage({action:"INTEGRATION:UI:POSTED_FLOWS"},'*');
        //                         if(reload){
