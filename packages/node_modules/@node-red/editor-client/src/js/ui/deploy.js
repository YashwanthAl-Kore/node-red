...
RED.deploy = (function() {
    ...
    function init(options) {
        ...
        /*$('<li>'+
            '<span class="red-ui-draft-button-group button-group">'+
                '<a id="red-ui-header-button-draft" class="red-ui-draft-button disabled" href="#">'+
                    '<span class="red-ui-draft-button-content">'+
                        '<span>'+"Draft"+'</span>'+
                    '</span>'+
                    '<span class="red-ui-draft-button-spinner hide">'+
                        '<img src="red/images/spin.svg"/>'+
                    '</span>'+
                '</a>'+
                '<a id="red-ui-header-button-draft-delete" class="red-ui-draft-button disabled" >' +
                    '<img src="red/images/delete-button-svg.svg">'+
                    '<span class="red-ui-delete-draft-button-spinner hide">'+
                        '<img src="red/images/spin.svg"/>'+
                    '</span>'+
                '</a>' +
            '</span>'+
        ...
        $('<li>' + 
            '<span class="red-ui-draft-button-group button-group">' + 
                '<a id="review-changes" class="red-ui-draft-button" href="#">'+
                    '<span class="red-ui-draft-button-content">'+
                        "Review Changes" +
                    '</span>'+
                '</a>'+
        ...
        const runtimeMsg = {
            DEPLOY: "nodered:runtime:deploy",
            DRAFT: "nodered:runtime:draft",
        }
        window.addEventListener('message', (event) => {
            ...
            if(action === runtimeMsg.DRAFT){
                saveDraft();
            }else if(action === runtimeMsg.DEPLOY){
                save();
            }
        });
        $('#red-ui-header-button-draft').on("click", function(event) {
            event.preventDefault();
            saveDraft();
        });
        $('#red-ui-header-button-draft-delete').on("click", function(event) {
            event.preventDefault();
            deleteDraft();
        });
        $('#review-changes').on('click',function(event){
            event.preventDefault();
        ...
        RED.events.on('workspace:dirty',function(state) {
            ...
            if (state.dirty) {
                ...
                $('#red-ui-header-button-draft').removeClass('disabled');
                $('#red-ui-header-button-draft-delete').removeClass('disabled');
            } else {
                $("#red-ui-header-button-deploy").addClass("disabled");
                $("#red-ui-header-button-draft").addClass("disabled");
                $("#red-ui-header-button-draft-delete").addClass("disabled");
            }
        });

        RED.events.on('workspace:draftDirty', function(state){
            if (RED.settings.user?.permissions === 'read') {
                return
            }
            if (state.draftDirty) {
        ...
        RED.events.on('workspace:deleteDraftDirty', function(state){
            if (RED.settings.user?.permissions === 'read') {
                return
            }
            if (state.deleteDraftDirty) {
                $('#red-ui-header-button-draft-delete').removeClass('disabled');
            } else {
                $("#red-ui-header-button-draft-delete").addClass("disabled");
            }
    ...
    function draftButtonSetBusy(){
        $(".red-ui-draft-button-content").css('opacity',0);
        $(".red-ui-draft-button-spinner").show();
        $("#red-ui-header-button-draft").addClass("disabled");
    }
    ...
    function draftButtonClearBusy(){
        $(".red-ui-draft-button-content").css('opacity',1);
        $(".red-ui-draft-button-spinner").hide();
    }
    ...
    function save(skipValidation, force) {
        ...
        $.ajax({
            ...
            RED.nodes.setDraftDirty(false);
            RED.nodes.setDeleteDraftDirty(false);
            ...
            window.parent.postMessage({action:"INTEGRATION:UI:FAILED_POST_FLOWS"},'*');
            $("#red-ui-header-button-deploy").removeClass("disabled");
            if (xhr.status === 401) {
                RED.notify(RED._("deploy.deployFailed", { message: RED._("user.notAuthorized") }), "error");
            } else if (xhr.status === 409) {
    ...
    function saveDraft(){
        if($("#red-ui-header-button-draft").hasClass("disabled")){
        ...
        const data = { flows: nns, rev: RED.nodes.version() };
        const appId = RED.utils.getAppId();
        const appType = RED.utils.getAppType();
        shadeShow();
        $.ajax({
            ...
            console.log("SENT ",{action:"INTEGRATION:UI:DRAFTED_FLOWS"});
            window.parent.postMessage({action:"INTEGRATION:UI:DRAFTED_FLOWS"},'*');
            RED.nodes.dirty(true);
            RED.nodes.setDraftDirty(false);
            RED.nodes.setDeleteDraftDirty(true);
            RED.nodes.version(data.rev);
            RED.nodes.originalFlow(nns);
            RED.notify('<p>' + RED._("deploy.successfulDrafted") + '</p>', "success");
            RED.history.markAllDirty();
            RED.view.redraw();
            RED.sidebar.config.refresh();
        }).fail(()=>{
            RED.nodes.dirty(true);
            window.parent.postMessage({action:"INTEGRATION:UI:FAILED_DRAFTED_FLOWS"},'*');
            $("#red-ui-header-button-draft").removeClass("disabled");
            if (xhr.status === 401) {
                RED.notify(RED._("deploy.deployFailed", { message: RED._("user.notAuthorized") }), "error");
            } else if (xhr.status === 409) {
            ...
            const delta = Math.max(0, 300 - (Date.now() - startTime));
            setTimeout(function () {
                draftButtonClearBusy()
                shadeHide()
            }, delta);
        })
    }
    async function deleteDraft(){
        // delete drafted flows 
        // disable deletedraftbutton 
        // disable deploy button
        // disable draft button
        // get deployed flows
        // set deployed flows
        if($("#red-ui-header-button-draft-delete").hasClass("disabled")){
            return; // already drafted
        }
        if (!RED.user.hasPermission("flows.write")) {
            RED.notify(RED._("user.errors.deploy"), "error");
        ...
        if (RED.workspaces.count() > 0) {
            const hiddenTabs = JSON.parse(
                RED.settings.getLocal("hiddenTabs") || "{}"
            );
