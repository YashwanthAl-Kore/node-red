...
RED.utils = (function() {
    ...
    /**
         * Create or update an icon element and append it to iconContainer.
         * @param {*} draftedFlows - Array of nodes.
         * @param {*} deployedFlows - Array of nodes.
         * @returns - Set of modified nodeId's
         */
    function diffNodes(draftedFlows, deployedFlows) {
        // isPopulated options   props
        draftedFlows = draftedFlows.map(flow => {
            const { isPopulated, options, props, ...rest } = flow;
            return rest;
        })
        deployedFlows = deployedFlows.map(flow => {
            const { isPopulated, options, props, ...rest } = flow;
            return rest;
        })
        if(!draftedFlows ||  (Array.isArray(draftedFlows) && !draftedFlows.length)){
            return new Set();
        }
        if(!draftedFlows ||  (Array.isArray(draftedFlows) && !draftedFlows.length)){
            return new Set(draftedFlows.map((flowNode) => flowNode.id))
        }
        const diffNodes = new Set();

        const drMap = new Map(draftedFlows.map((flow) => [flow.id, flow]));
        const deMap = new Map(deployedFlows.map((flow) => [flow.id, flow]));


        for (const [id, drFlow] of drMap) {
            const deFlow = deMap.get(id);
            if (!deFlow || !deepEqual(drFlow, deFlow)) {
                diffNodes.add(id);
                if(drFlow.type != "tab"){
                    diffNodes.add(drFlow.z)
                }
            }
        }
