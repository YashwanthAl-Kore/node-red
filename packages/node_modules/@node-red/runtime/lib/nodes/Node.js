...
const LogEmitter = require('../../../../node-red/utils/log-emitter');
...
function Node(n) {
    ...
    this.tenantConfigId = n.tenantConfigId;
    this.appId = n.appId;
    this.execStartTime = 0;
    this.execCompletionTime = 0;
    this.currentExecutionId = null;
    this.currentTriggeredBy = null;
    this.currentIsStartNode = null;
    this.appType = n.appType;
}
...
Node.prototype._complete = function (msg, error) {
    ...
    this.execCompletionTime = Date.now();
    var isEndNode = this.wires.length === 0 || this.wires.every(wire => wire.length === 0);
    var nodeOut = _.toPlainObject(msg)
    if (nodeOut.error || (nodeOut.statusCode && (isNaN(nodeOut.statusCode) || nodeOut.statusCode >= 400 || nodeOut.statusCode < 200))) {
        msg.errorPresent = true
    }
    const metricRecord = {
        tenantConfigId: this.tenantConfigId,
        appId: this.appId,
        id: msg._msgid,
        nodeId: this.id,
        nodeType: this.type,
        nodeIn: this.nodeIn,
        nodeOut,
        startTime: this.execStartTime,
        endTime: this.execCompletionTime,
        name: this.name,
        appType: this.appType,
        isStartNode: this.currentIsStartNode,
        isLastNode: isEndNode,
        isErrorPresent: msg.errorPresent
    }
    logEmitter.emit('runTimeLog', metricRecord)
...
Node.prototype.receive = function (msg) {
    ...
    if (!msg.executionId && !msg.triggeredBy) {
        msg.executionId = redUtil.generateId();
    }
    this.execStartTime = Date.now();
    this.nodeIn = _.toPlainObject(msg);
    this.currentIsStartNode = !msg.triggeredBy;
    this.currentExecutionId = msg.executionId;
    this.currentTriggeredBy = msg.triggeredBy;
    this.emit("input", msg);
};
...
Node.prototype.metric = function (eventname, msg, metricValue) {
    ...
    metrics.executionId = msg.executionId;
    metrics.value = metricValue;
    metrics.isStartNode = msg.isStartNode;
    metrics.isEndNode = this.wires.length === 0 || this.wires.every(wire => wire.length === 0);
    metrics.triggeredBy = msg.triggeredBy;
    if (msg.error) metrics.error = msg.error;
...
function logErrorMetric(node, error) {
    if (node && node.metric) {
        const metricMsg = {
            _msgid: redUtil.generateId(),
            executionId: node.currentExecutionId,
            triggeredBy: node.currentTriggeredBy,
            isStartNode: node.currentIsStartNode,
            error: error instanceof Error ? error.toString() : error,
            timestamp: Date.now()
        };
        node.metric("unexpected_error", metricMsg);
    }
}
