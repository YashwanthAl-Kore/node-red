    ...
    Object.defineProperties(obj, {
        get: {
            value: function(key, storage, callback) {
                ...
                if (callback) {
                    if (!seed) {
                        context.get(scope, key, {tenantConfigId: this?.tenantConfigId, appId: this?.appId}, callback);
                    } else {
                        context.get(scope, key,  {tenantConfigId: this?.tenantConfigId, appId: this?.appId}, function() {
                            if (arguments[0]) {
                    ...
                    var results = context.get(scope, key, {tenantConfigId: this?.tenantConfigId, appId: this?.appId});
                    if (seed) {
                        if (Array.isArray(key)) {
                            insertSeedValues(key,results);
        ...
        set: {
            value: function(key, value, storage, callback) {
                ...
                context.set(scope, key, value, {tenantConfigId: this?.tenantConfigId, appId: this?.appId}, callback);
            }
        },
        keys: {
            value: function(storage, callback) {
                ...
                if (seed && settings.exportGlobalContextKeys !== false) {
                    if (callback) {
                        context.keys(scope, {tenantConfigId: this?.tenantConfigId, appId: this?.appId}, function(err,keys) {
                            callback(err,Array.from(new Set(seedKeys.concat(keys)).keys()));
                        });
                    } else {
                        var keys = context.keys(scope, {tenantConfigId: this?.tenantConfigId, appId: this?.appId});
                        return Array.from(new Set(seedKeys.concat(keys)).keys())
                    }
                } else {
                    return context.keys(scope, {tenantConfigId: this?.tenantConfigId, appId: this?.appId}, callback);
                }
            }
        }
    });
