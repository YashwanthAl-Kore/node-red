...
const { redisClient, redisChannelManager, CHANNELS, EVENTS, FLOWS_REGISTERY_TYPES } = require('node-red/utils/redis');
...
function init(runtime) {
    ...
    redisClient.on(EVENTS.FLOW_UPDATE, async (message) => {
        try {
            const data = JSON.parse(message);
            log.debug(`Received flow update`);
            await _updateFlows(data.opts, true);
        } catch (err) {
            log.error("Error processing flow update message: " + err.message);
        }
    });

    redisClient.on(EVENTS.FLOWS_REGISTERY, async (message) => {
        try {
            const { tenantConfigId, appId, type } = JSON.parse(message);
            if (!tenantConfigId || !appId) {
                log.info("Invalid message for FLOWS_REGISTRY:", message);
                return;
            }
            const { flows } = await storage.getFlows(tenantConfigId, appId);
            ...
            switch (type) {
                case FLOWS_REGISTERY_TYPES.REGISTER:
                    diffObj.changed = flowIds;
                    await storage.updateFlowsRecords(tenantConfigId, appId, { disabled: false })
                    await start("flows", diffObj, null, true);
                    log.info("starting flows completed");
                    break;
                case FLOWS_REGISTERY_TYPES.UNREGISTER:
                    diffObj.removed = flowIds;
                    await storage.updateFlowsRecords(tenantConfigId, appId, { disabled: true })
                    await stop("flows", diffObj, null, true);
                    log.info("stopping flows completed");
                    break;
                case FLOWS_REGISTERY_TYPES.DELETE:
                    diffObj.removed = flowIds;
                    await storage.updateFlowsRecords(tenantConfigId, appId, { recStatus: 'inactive', disabled: true })
                    await stop("flows", diffObj, null, true);

                    log.info('stop and deleting flows completed')
                    break;
                default:
                    throw Error("Invalid type specified");
            }
        } catch (error) {
            log.error("Error in FLOWS_REGISTER handler: " + error.message);
        }
    });
...
async function updateFlows(opts) {
    return _updateFlows(opts, false);
}

async function _updateFlows(opts, skipPublication) {
    // opts : { tenantConfigId: "" , appId: "" , flows: []}
    const activeFlowsOfTenant = await getFlows(opts.tenantConfigId, opts.appId, opts.appType);
    currentFlowConfig = flowUtil.parseConfig(activeFlowsOfTenant.flows);

    await storage.deleteFlows(opts.tenantConfigId, opts.appId, opts.appType);
    await storage.deleteDrafts(opts.tenantConfigId, opts.appId, opts.appType);
    let newRev = await storage.saveFlows(opts.flows, opts.tenantConfigId, opts.emailId, opts.appId, opts.appType);

    const newFlowConfig = flowUtil.parseConfig(opts.flows);
    diff = flowUtil.diffConfigs(currentFlowConfig, newFlowConfig);

    const flows = await storage.getFlows(opts.tenantConfigId, opts.appId, opts.appType);
    activeFlowConfig = flowUtil.parseConfig(flows.flows);
    try {
        await stop("flows", diff, null, true);
        await start("flows", diff, null, true);
    } catch (err) {
        log.error("Error updating flows:", err.message);
        throw err;
    }
    ...
    if (!skipPublication) {
        ...
        await redisChannelManager.publish(CHANNELS.FLOW_UPDATE, message);
        log.debug(`Published flow update to ${CHANNELS.FLOW_UPDATE}`);
    }
...
function setFlows(_config, _credentials, type, muteLog, forceStart, user) {
    ...
    if (type === "load") {
        ...
        // flows should be set here at application starting process
...
async function getFlows(tenantConfigId, appId, appType) {
    const flows = await storage.getFlows(tenantConfigId, appId, appType);
    return flows;
}
