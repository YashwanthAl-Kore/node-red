...
module.exports = function(RED) {
    ...
    function buildTimeoutOptions(opts){
        const defaultOptions = {
            timeout: (RED.settings.get("functionTimeout") || 2) * 1000, // default timeout is 2 seconds
            breakOnSigint: true
        };
        ...
        Promise.all(moduleLoadPromises).then(() => {
            ...
            try {
                ...
                if (iniScript) {
                    ...
                    promise = iniScript.runInContext(context, buildTimeoutOptions(iniOpt));
                }
                ...
                processMessage = function (msg, send, done) {
                    ...
                    try {
                        node.script.runInContext(context,buildTimeoutOptions(opts));
                    } catch (err) {
                        if (err instanceof Error && err.name === 'TimeoutError') {
                            return Promise.reject(new Error(`Function execution timed out after ${opts.timeout || 2000}ms`));
                        }
                        node.error(err);
                       // return done(err);
                    }
                ...
                node.on("close", function() {
                    if (finScript) {
                        try {
                            finScript.runInContext(context, buildTimeoutOptions(finOpt));
                        }
                        catch (err) {
                            node.error(err);
                        }
