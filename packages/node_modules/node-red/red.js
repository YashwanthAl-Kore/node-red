...
httpsPromise.then(function(startupHttps) {
    ...
    //     }
    // }
    settings.uiPort = process.env.PORT || 3030 || 1880;
    console.log('>>>>>>>>>>>>>> settings.uiPort >>>>>>>>>>>>>>>>>', settings.uiPort);
    ...
    if (settings.httpInternalRoot !== false) {
        app.use(settings.httpInternalRoot,RED.httpInternalApp);
    }
    ...
    RED.start().then(function() {
        ...
        if (settings.httpAdminRoot !== false || settings.httpNodeRoot !== false || settings.httpStatic) {
            ...
            server.listen(settings.uiPort,settings.uiHost,function() {
                ...
                console.log('>>>>>>>>>>>>>> server.now-running >>>>>>>>>>>>>>>>>', getListenPath());
            });
        } else {
